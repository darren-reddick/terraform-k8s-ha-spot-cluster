...
EOF
  ...
  provisioner "local-exec" {
    command =<<EOF
aws ec2 create-tags --resources ${self.spot_instance_id} --tags Key=Name,Value=Kubeworker${count.index}
aws ec2 create-tags --resources ${self.spot_instance_id} --tags Key=kubernetes.io/cluster/${var.stackname},Value=owned
aws ec2 create-tags --resources ${self.spot_instance_id} --tags Key=Kuberole,Value=worker
